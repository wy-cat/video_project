...
// ==============================
function getFirstLastFrames(imageUrls) {
    try {
...
function selectStepCard(step) {
    ...
    console.log(`📌 选择步骤 ${step} - ${STEP_LABELS[step] || '未知步骤'}`);
    // 更新卡片高亮
...
function renderStepContent(step) {
    ...
    const stepName = STEP_LABELS[step] || `步骤${step}`;
    console.log("所有渲染内容", stepOutputs);

    // ==========================================
    // 🔥 唯一正确：只有 content === null → 正在生成
    // ==========================================
    if (content === null) {
        contentContainer.innerHTML = `<p style="color:#007bff; font-weight:600;">⏳ 正在生成【${stepName}】...</p>`;
        return;
    }
    ...
    if (content) {
        ...
        let html = '<div class="step-output-header">';
        ...
        if ((isEditable || isRegeneratable) && rawData) {
            ...
            if (!isEditing) {
                if (isRegeneratable) html += `<button class="regenerate-btn" onclick="regenerateStep(${step})">🔄 重新生成</button>`;
                if (isEditable) html += `<button class="edit-btn" onclick="enterEditMode(${step})">✏️ 编辑</button>`;
            } else {
...
function formatStepResult(step, result) {
    ...
    switch (stepNum) {
        ...
        case 7: // 拼接视频
            return formatVideoEditingResult(result); // 目前没有具体数据结构，先简单展示
        default:
...
function formatShotScriptResult(result) {
    ...
    html += '<h4>场景细节</h4>';
    html += `<p>${typeof scene_detail === 'object' ? JSON.stringify(scene_detail, null, 2) : scene_detail || '无'}</p>`;
    html += '</div>';
    ...
    if (roles && Array.isArray(roles)) {
        ...
        roles.forEach(role => {
            // 关键修复：强制转为字符串
            const roleText = typeof role === 'object' ? JSON.stringify(role, null, 2) : role;
            html += `<li>${roleText}</li>`;
        });
        html += '</ul>';
    } else {
...
function formatImagesResult(result) {
    ...
    imageUrls.forEach((url, idx) => {
        ...
        html += `<img src="${url}" alt="图片${idx + 1}">`;
        html += `<div class="media-caption" onclick="showImagePrompt(${idx}, '${escapeHtml(prompt)}')">图片 ${idx + 1}</div>`;
        html += `</div>`;
    });
    html += '</div>';
    return html;
...
function formatVideosResult(result) {
    ...
    videoUrls.forEach((url, idx) => {
        ...
        html += `<div class="media-caption" onclick="showVideoInfo(${idx}, '${escapeHtml(prompt)}', '${firstFrame}', '${lastFrame}')">视频 ${idx + 1}</div>`;
        html += `</div>`;
    });
    html += '</div>';
    return html;
...
// 步骤7：视频结果展示（目前没有具体数据结构，先简单展示）
function formatVideoEditingResult(result) { }
...
async function loadTasksList() {
    try {
        ...
        data.tasks.forEach(task => {
            ...
            // 根据是否需要确认来设置不同的样式和文本
            let buttonStyle, buttonText, buttonOnClick, buttonDisabled;
            ...
            if (isPending) {
                ...
                `;
                buttonText = `⏸️ 核对完成，继续生成(${window.pendingConfirmation.nextStep})`;
    ...
    try {
        ...
        completedSteps.sort((a, b) => a - b).forEach(step => {
            ...
            switch (step) {
                ...
                case 6:
                    if (stepData.videoUrls) {
                        // 需要获取步骤5的提示词和步骤4的首尾帧图片
                        const step4Data = task.steps[4]?.data;
                        const step5Data = task.steps[5]?.data;

                        // 先调用函数，拿到返回的 { firstFrames, lastFrames }
                        const { firstFrames, lastFrames } = getFirstLastFrames(step4Data?.imageUrls || []);
...
document.getElementById('generateBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        eventSource.onmessage = (event) => {
            ...
            // 1. 步骤正在生成 → 显示加载
            if (data.step && data.result === null && stepOutputs[data.step] === undefined) {
                stepOutputs[data.step] = null; // 标记为【正在生成】
                updateStepCards();
                selectStepCard(data.step);
            }

            // 2. ✅ 步骤生成完成 → 强制渲染内容、标记完成（最优先）
            if (data.step && data.message.includes('✅') && data.result) {
                setStepCompleted(data.step);
                addStepOutput(data.step, data.message, data.result, true);
                selectStepCard(data.step);
            }

            // 3. ⏸️ 等待用户确认 → 只处理逻辑，不碰界面
            if (data.result?.waitingForConfirmation) {
...
async function continueFromStep(taskId, startStep) {
    ...
    // 初始化：前面步骤=已完成,当前步骤=生成中
    const initProgressNodes = (startStep) => {
        ...
        setStepActive(startStep);
        console.log(`已初始化步骤状态：步骤${startStep - 1}标记为已完成，步骤${startStep}标记为生成中`);
    };
